...
import { mockAdvertisers, mockAgencies } from "@/lib/mock-data";
import { use, useState } from "react";
import { useRouter } from "next/navigation";

export default function AdvertiserEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [formData, setFormData] = useState({
    ...
    agencyContactName: advertiserData.agencyContactName || "",
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // 詳細ページに戻る
    router.push(`/admin/advertisers/${resolvedParams.id}`);
  };
    ...
    <AdminLayout>
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-4 mb-6">
          <Link
            href={`/admin/advertisers/${resolvedParams.id}`}
            className="p-2 hover:bg-gray-100 rounded-lg transition-colors"
          >
            <ArrowLeft className="h-5 w-5" />
          </Link>
          <div>
            <h1 className="text-2xl font-bold text-gray-900">広告主情報編集</h1>
          ...
          <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      広告主NO <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="text"
                      ...
                      required
                    />
                    <p className="text-xs text-gray-500 mt-1">英数字・ハイフン・アンダーバー可</p>
                  </div>
                ...
                {[1, 2, 3, 4, 5].map((num) => (
                  <div key={num}>
                    ...
                    <textarea
                      ...
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent bg-red-50"
            ...
            <div className="flex items-center justify-between">
              <p className="text-sm text-gray-500">
                登録日: {advertiser.registeredDate ? new Date(advertiser.registeredDate).toLocaleString('ja-JP') : "—"}
              </p>
