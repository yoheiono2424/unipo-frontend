...
export default function CategoryEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // 親カテゴリ候補（自分自身と自分の子カテゴリは除く）
  const parentCandidates = mockCategories.filter(cat =>
    cat.parentId === null && cat.id !== category.id
  );
    ...
    <AdminLayout>
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-4 mb-6">
          <Link
            href={`/admin/categories/${resolvedParams.id}`}
            className="p-2 hover:bg-gray-100 rounded-lg transition-colors"
          >
            <ArrowLeft className="h-5 w-5" />
          </Link>
          <div>
            <h1 className="text-2xl font-bold text-gray-900">カテゴリ編集</h1>
            ...
            <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                    >
                      <option value="">親カテゴリとして設定</option>
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                ...
                <ul className="text-xs text-yellow-700 space-y-1">
                  <li>• このカテゴリに属する店舗がある場合、カテゴリ変更は慎重に行ってください</li>
                  <li>• 子カテゴリがある場合、親カテゴリを他のカテゴリの子に変更することはできません</li>
                  <li>• カテゴリ名を変更すると、関連する店舗・商品の表示にも影響します</li>
                </ul>
              </div>
